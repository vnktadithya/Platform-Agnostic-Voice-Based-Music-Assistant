...
class UserPlaylist(Base):
    ...
    meta_data = Column(JSON, nullable=True)  # ✅ For any platform-specific playlist data
...
class UserLikedSong(Base):
    ...
    meta_data = Column(JSON, nullable=True)  # ✅ For any platform-specific track data
    last_synced = Column(DateTime, default=datetime.datetime.now(datetime.timezone.utc))
...
class SearchCache(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    platform_account_id = Column(Integer, ForeignKey("platform_accounts.id"))
    normalized_query = Column(String, index=True)  # Lowercased, punctuation-stripped user query
    track_uri = Column(String, index=True)
    meta_data = Column(JSON)  # Stores artist, title, album, etc.
