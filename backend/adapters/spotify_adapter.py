...
# Setup basic logging to see the flow in your terminal
...
class SpotifyAdapter(MusicPlatformAdapter):
    ...
    def __init__(self, access_token: str):
        ...
        logger.info(f"SpotifyAdapter initialized for ACTION with token ...{access_token[-4:]}")
    ...
    def play(self, uris: list[str]):
        ...
        logger.info(f"Attempting to play URIs on device {device_id}")
        self.sp.start_playback(device_id=device_id, uris=uris)
        logger.info("Spotify 'start_playback' command sent successfully.")

    def pause(self):
        device_id = self._get_active_device_id()
        logger.info(f"Attempting to pause playback on device {device_id}")
        self.sp.pause_playback(device_id=device_id)
        logger.info("Spotify 'pause_playback' command sent successfully.")

    def resume(self):
        device_id = self._get_active_device_id()
        logger.info(f"Attempting to resume playback on device {device_id}")
        self.sp.start_playback(device_id=device_id)
        logger.info("Spotify 'resume' command sent successfully.")

    def skip_current_track(self):
        device_id = self._get_active_device_id()
        logger.info(f"Attempting to skip track on device {device_id}")
        self.sp.next_track(device_id=device_id)
        logger.info("Spotify 'next_track' command sent successfully.")

    def previous_track(self):
        device_id = self._get_active_device_id()
        logger.info(f"Attempting to go to previous track on device {device_id}")
        self.sp.previous_track(device_id=device_id)
        logger.info("Spotify 'previous_track' command sent successfully.")

    def restart_current_track(self):
        device_id = self._get_active_device_id()
        logger.info(f"Attempting to restart track on device {device_id}")
        self.sp.seek_track(0, device_id=device_id)
        logger.info("Spotify 'seek_track' to 0ms command sent successfully.")
    ...
    def add_current_to_favorites(self, db: Session, platform_account_id: int):
        ...
        if not current or not current.get("item"):
            logger.warning("⚠️ No current playback track found.")
            return
        ...
        logger.info(f"❤️ Liking track on Spotify: {track['name']}")
    ...
    def remove_current_from_favorites(self, db: Session, platform_account_id: int):
        ...
        current = self.sp.current_playback()
        if not current or not current.get("item"):
            logger.warning("⚠️ No track currently playing.")
            return
        ...
        if existing:
            db.delete(existing)
            db.commit()
            logger.info(f"🗑️ Removed '{track['name']}' from local cache.")
    ...
    def search_track_uris(self, db: Session, platform_account_id: int, query: str, limit: int = 1) -> list:
        """
        Search tracks by user query: fuzzy match SearchCache, fallback to Spotify API and cache result.
        """
        ...
        if match:
            logger.info(f"✅ Fuzzy SearchCache hit: '{match.normalized_query}' (score: {score})")
            return [match.track_uri]

        logger.warning(f"⚠️ No SearchCache match for '{query}'. Querying Spotify API.")
        results = self.sp.search(q=query, type="track", limit=limit)
        tracks = results.get("tracks", {}).get("items", [])
        uris = [track['uri'] for track in tracks]

        # Cache top result for future searches
        if tracks:
            track = tracks[0]
            db.add(SearchCache(
                platform_account_id=platform_account_id,
            ...
            db.commit()
            logger.info(f"🆕 Cached search result '{norm_query}' → '{track.get('name')}' in SearchCache.")

        return uris


    def play_by_query(self, db: Session, platform_account_id: int, query: str, limit: int = 5):
        uris = self.search_track_uris(db, platform_account_id, query, limit=limit)
        if uris:
            self.play(uris=uris)
